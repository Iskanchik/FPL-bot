...
#
# Включены последние правки:
# - League name вместо "League" (fallback).
# - Таблицы без столбца Team (entry_name) — оставлены только имена менеджеров.
# - DC показывается как DC+2 в /gwinfo; пояснение "DC=Defensive contribution (+2)".
# - /month принимает номер сезонного месяца 1..10 (Август..Май) и показывает соответствующий календарный месяц.
# - Squid Game: имена менеджеров вместо Entry ID.
# - Усиленный парсер /prices: сначала Next.js JSON (__NEXT_DATA__), затем HTML/regex фоллбек; фильтр ownership >=1%.
# - Исправлена SyntaxError (global _bootstrap_etag перенесён в начало get_bootstrap_cached).
# - Кэш /prices 10 мин, ежедневный автопост в 23:00 Asia/Almaty.
# - Idempotent отправка сообщений (Redis SADD + expire или память).
#
...
def parse_prices(html: str, ownership_map: Dict[str, float]) -> Dict[str, Dict[str, List[Tuple[str, str]]]]:
    ...
    def extract(section_text: str) -> Dict[str, List[Tuple[str, str]]]:
        ...
        for cat in categories:
            part = re.search(cat + r"(.*?)(?:Already reached target|Projected to reach today|Close by end of day|Rises|Falls|$)",
                             section_text, re.I | re.S)
            if part:
                chunk = part.group(1)
        ...
        try:
            ...
            for fid in sorted(set(list(pos_pool.keys()) + list(neg_pool.keys()))):
                ...
                while own_goals:
                    ...
                    line1 = f"{mstr} Own goal - {PLAYER_NAME_MAP.get(ogid, f'P{ogid}')} {format_points(pts_ог)}, total ({total_og})."
                    if a_pid:
                        pos_a, _m_a, total_a = get_state(a_pid)
                        pts_a = event_points_for(pos_a, "Assist")
                        line2 = f"Assist - {PLAYER_NAME_MAP.get(a_pid, f'P{a_pid}')} {format_points(pts_a)}, total ({total_a})."
